...
namespace osu.Game.Rulesets.Sanko.Tests
{
    public partial class TestSceneReplayRecording : PlayerTestScene
    {
        ...
        public void TestRecording()
        {
            ...
            AddAssert("left rim press recorded to replay", () => Player.Score.Replay.Frames.OfType<SankoReplayFrame>().Any(f => f.Actions.SequenceEqual([SankoAction.LeftRim])));

            seekTo(5000);
            AddStep("press F", () => InputManager.PressKey(Key.F));
            seekTo(5015);
            AddStep("release F", () => InputManager.ReleaseKey(Key.F));
            AddAssert("left centre press recorded to replay", () => Player.Score.Replay.Frames.OfType<SankoReplayFrame>().Any(f => f.Actions.SequenceEqual([SankoAction.LeftCentre])));

            seekTo(10000);
            AddStep("press J", () => InputManager.PressKey(Key.J));
            seekTo(10015);
            AddStep("release J", () => InputManager.ReleaseKey(Key.J));
            AddAssert("right centre press recorded to replay", () => Player.Score.Replay.Frames.OfType<SankoReplayFrame>().Any(f => f.Actions.SequenceEqual([SankoAction.RightCentre])));

            seekTo(15000);
            AddStep("press K", () => InputManager.PressKey(Key.K));
            seekTo(15015);
            AddStep("release K", () => InputManager.ReleaseKey(Key.K));
            AddAssert("right rim press recorded to replay", () => Player.Score.Replay.Frames.OfType<SankoReplayFrame>().Any(f => f.Actions.SequenceEqual([SankoAction.RightRim])));
        }
