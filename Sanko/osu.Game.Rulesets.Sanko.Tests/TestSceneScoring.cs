...
namespace osu.Game.Rulesets.Sanko.Tests
{
    ...
    public partial class TestSceneScoring : ScoringTestScene
    {
        ...
        protected override ProcessorBasedScoringAlgorithm CreateScoreAlgorithm(IBeatmap beatmap, ScoringMode mode, IReadOnlyList<Mod> selectedMods)
            => new SankoProcessorBasedScoringAlgorithm(beatmap, mode, selectedMods);
        ...
        private class SankoProcessorBasedScoringAlgorithm : ProcessorBasedScoringAlgorithm
        {
            public SankoProcessorBasedScoringAlgorithm(IBeatmap beatmap, ScoringMode mode, IReadOnlyList<Mod> selectedMods)
                : base(beatmap, mode, selectedMods)
            {
            }

            protected override ScoreProcessor CreateScoreProcessor() => new SankoScoreProcessor();
            protected override JudgementResult CreatePerfectJudgementResult() => new JudgementResult(new Hit(), new SankoJudgement()) { Type = HitResult.Great };
            protected override JudgementResult CreateNonPerfectJudgementResult() => new JudgementResult(new Hit(), new SankoJudgement()) { Type = HitResult.Ok };
            protected override JudgementResult CreateMissJudgementResult() => new JudgementResult(new Hit(), new SankoJudgement()) { Type = HitResult.Miss };
        }
    }
}
