...
namespace osu.Game.Rulesets.Sanko.Difficulty.Evaluators
{
    public static class StaminaEvaluator
    {
        ...
        public static double EvaluateDifficultyOf(DifficultyHitObject current)
        {
            ...
            // available fingers.
            SankoDifficultyHitObject sankoCurrent = (SankoDifficultyHitObject)current;
            SankoDifficultyHitObject? sankoPrevious = current.Previous(1) as SankoDifficultyHitObject;
            SankoDifficultyHitObject? previousMono = sankoCurrent.PreviousMono(availableFingersFor(sankoCurrent) - 1);

            double objectStrain = 0.5; // Add a base strain to all objects
            if (sankoPrevious == null) return objectStrain;

            if (previousMono != null)
                objectStrain += speedBonus(sankoCurrent.StartTime - previousMono.StartTime) + 0.5 * speedBonus(sankoCurrent.StartTime - sankoPrevious.StartTime);
        ...
        /// <summary>
        /// Determines the number of fingers available to hit the current <see cref="SankoDifficultyHitObject"/>.
        ...
        /// </summary>
        private static int availableFingersFor(SankoDifficultyHitObject hitObject)
        {
