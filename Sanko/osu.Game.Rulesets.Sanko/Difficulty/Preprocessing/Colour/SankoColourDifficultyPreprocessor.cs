...
namespace osu.Game.Rulesets.Sanko.Difficulty.Preprocessing.Colour
{
    ...
    /// </summary>
    public static class SankoColourDifficultyPreprocessor
    {
        /// <summary>
        /// Processes and encodes a list of <see cref="SankoDifficultyHitObject"/>s into a list of <see cref="SankoColourData"/>s,
        /// assigning the appropriate <see cref="SankoColourData"/>s to each <see cref="SankoDifficultyHitObject"/>.
        /// </summary>
        ...
        /// <summary>
        /// Encodes a list of <see cref="SankoDifficultyHitObject"/>s into a list of <see cref="RepeatingHitPatterns"/>s.
        /// </summary>
        ...
        /// <summary>
        /// Encodes a list of <see cref="SankoDifficultyHitObject"/>s into a list of <see cref="MonoStreak"/>s.
        /// </summary>
        private static List<MonoStreak> encodeMonoStreak(List<DifficultyHitObject> data)
        {
            ...
            {
                SankoDifficultyHitObject sankoObject = (SankoDifficultyHitObject)data[i];

                // This ignores all non-note objects, which may or may not be the desired behaviour
                SankoDifficultyHitObject? previousObject = sankoObject.PreviousNote(0);

                // If this is the first object in the list or the colour changed, create a new mono streak
                if (currentMonoStreak == null || previousObject == null || (sankoObject.BaseObject as Hit)?.Type != (previousObject.BaseObject as Hit)?.Type)
                {
                ...
                currentMonoStreak.HitObjects.Add(sankoObject);
            }
        ...
        private static List<RepeatingHitPatterns> encodeRepeatingHitPattern(List<AlternatingMonoPattern> data)
        {
            ...
            for (int i = 0; i < data.Count; i++)
            {
                ...
                // Note: sanko does not attempt to detect longer (e.g. three-colour) cycles here. Real beatmaps contain
                // sections which repeat every three patterns even when only two colours are used, and grouping those
                // changes the resulting difficulty away from the osu!taiko baseline this model is calibrated against.
