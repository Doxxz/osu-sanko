...
{
    internal class SankoLegacyScoreSimulator : ILegacyScoreSimulator
    {
        private readonly ScoreProcessor scoreProcessor = new SankoScoreProcessor();
        ...
        private void simulateHit(HitObject hitObject, HitObject? nextHitObject, ref LegacyScoreAttributes attributes)
        {
            ...
            switch (hitObject)
            {
                ...
                case Swell swell:
                    // The sanko swell generally does not match the osu-stable implementation in any way.
                ...
                case DrumRoll drumRoll:
                    double minHitDelay = getSliderSankoMinHitDelay(drumRoll);
                    ...
                    double? nextObjectHittableStartTime = nextHitObject is DrumRoll nextDrumRoll ? nextDrumRoll.StartTime - getSliderSankoMinHitDelay(nextDrumRoll) : nextHitObject?.StartTime;
            ...
            if (hitObject is Swell || (hitObject is SankoStrongableHitObject strongable && strongable.IsStrong))
            {
                scoreIncrease *= 2;
        ...
        private double getSliderSankoMinHitDelay(DrumRoll drumRoll)
        {
            double maxRate;
        ...
        public double GetLegacyScoreMultiplier(IReadOnlyList<Mod> mods, LegacyBeatmapConversionDifficultyInfo difficulty)
        {
            ...
            foreach (var mod in mods)
            {
                switch (mod)
                {
                    ...
                    case SankoModHalfTime:
                    case SankoModDaycore:
                        multiplier *= 0.3;
                        break;

                    case SankoModHidden:
                    case SankoModHardRock:
                        multiplier *= 1.06;
                        break;

                    case SankoModDoubleTime:
                    case SankoModNightcore:
                    case SankoModFlashlight:
                        multiplier *= 1.12;
                        break;
