...
namespace osu.Game.Rulesets.Sanko.Edit.Checks
{
    public class CheckSankoInconsistentSkipBarLine : ICheck
    {
        ...
        public IEnumerable<Issue> Run(BeatmapVerifierContext context)
        {
            ...
            // Inconsistent bar line omission only matters for osu!sanko difficulties, so only check those
            var sankoBeatmaps = context.AllDifficulties.Where(b => b.Playable.BeatmapInfo.Ruleset.ShortName == "sanko").ToList();

            if (sankoBeatmaps.Count <= 1)
                yield break;
            ...
            var otherSankoBeatmaps = sankoBeatmaps.Where(b => b.Playable != referenceBeatmap).ToList();

            foreach (var beatmap in otherSankoBeatmaps)
            {
