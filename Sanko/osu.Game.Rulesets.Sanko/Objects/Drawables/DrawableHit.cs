...
namespace osu.Game.Rulesets.Sanko.Objects.Drawables
{
    public partial class DrawableHit : DrawableSankoStrongableHitObject<Hit, Hit.StrongNestedHit>
    {
        /// <summary>
        /// A list of keys which can result in hits for this HitObject.
        /// </summary>
        public SankoAction[] HitActions { get; internal set; }

        /// <summary>
        /// The action that caused this <see cref="DrawableHit"/> to be hit.
        /// </summary>
        public SankoAction? HitAction
        {
            get;
            private set;
        }
        ...
        private SankoRulesetConfigManager sankoConfig { get; set; }
        ...
        private void load()
        {
            sankoConfig?.BindWith(SankoRulesetSetting.RateAdjustedHitAnimation, rateAdjustedHitAnimations);
            sankoConfig?.BindWith(SankoRulesetSetting.HitAnimations, hitAnimations);
        }
        ...
        protected override void RecreatePieces()
        {
            ...
            base.RecreatePieces();
            Size = new Vector2(HitObject.IsStrong ? SankoStrongableHitObject.DEFAULT_STRONG_SIZE : SankoHitObject.DEFAULT_SIZE);
        }
        ...
        {
            HitActions = HitObject.Type switch
            {
                HitType.Rim => new[] { SankoAction.LeftRim, SankoAction.RightRim },
                HitType.Tsu => new[] { SankoAction.LeftTsu, SankoAction.RightTsu },
                _ => new[] { SankoAction.LeftCentre, SankoAction.RightCentre },
            };
        }

        protected override SkinnableDrawable CreateMainPiece() => HitObject.Type switch
        {
            HitType.Rim => new SkinnableDrawable(new SankoSkinComponentLookup(SankoSkinComponents.RimHit), _ => new RimHitCirclePiece(), confineMode: ConfineMode.ScaleToFit),
            HitType.Tsu => new SkinnableDrawable(new SankoSkinComponentLookup(SankoSkinComponents.TsuHit), _ => new TsuHitCirclePiece(), confineMode: ConfineMode.ScaleToFit),
            _ => new SkinnableDrawable(new SankoSkinComponentLookup(SankoSkinComponents.CentreHit), _ => new CentreHitCirclePiece(), confineMode: ConfineMode.ScaleToFit),
        };
        ...
        public override bool OnPressed(KeyBindingPressEvent<SankoAction> e)
        {
        ...
        public override void OnReleased(KeyBindingReleaseEvent<SankoAction> e)
        {
            ...
            switch (state)
            {
                ...
                case ArmedState.Hit:
                    if (!hitAnimations.Value)
                    {
                        ...
                        // otherwise gameplay will never complete (see also: `SankoModHidden.ApplyNormalVisibilityState()`).
