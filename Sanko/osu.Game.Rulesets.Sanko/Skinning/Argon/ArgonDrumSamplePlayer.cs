...
namespace osu.Game.Rulesets.Sanko.Skinning.Argon
{
    internal partial class ArgonDrumSamplePlayer : DrumSamplePlayer
    {
        ...
        private void load(Playfield playfield, IPooledSampleProvider sampleProvider)
        {
            ...
            sampleProvider.GetPooledSample(new VolumeAwareHitSampleInfo(new HitSampleInfo(HitSampleInfo.HIT_WHISTLE), true));
            sampleProvider.GetPooledSample(new VolumeAwareHitSampleInfo(new HitSampleInfo(HitSampleInfo.HIT_FLOURISH), true));

            // Warm up pools for the samples tsu uses via the base trigger source.
            sampleProvider.GetPooledSample(new HitSampleInfo(HitSampleInfo.HIT_WHISTLE));
            sampleProvider.GetPooledSample(new HitSampleInfo(HitSampleInfo.HIT_FINISH));
