...
{
    public partial class DrawableSankoMascot : BeatSyncedContainer
    {
        public readonly Bindable<SankoMascotAnimationState> State;
        public readonly Bindable<JudgementResult?> LastResult;

        private readonly Dictionary<SankoMascotAnimationState, SankoMascotAnimation> animations;

        private SankoMascotAnimation? currentAnimation;

        private bool lastObjectHit = true;
        private bool kiaiMode;

        public DrawableSankoMascot(SankoMascotAnimationState startingState = SankoMascotAnimationState.Idle)
        {
            Origin = Anchor = Anchor.BottomLeft;

            State = new Bindable<SankoMascotAnimationState>(startingState);
            LastResult = new Bindable<JudgementResult?>();

            animations = new Dictionary<SankoMascotAnimationState, SankoMascotAnimation>();
        }
        ...
        private void load(GameplayState? gameplayState)
        {
            ...
            {
                animations[SankoMascotAnimationState.Idle] = new SankoMascotAnimation(SankoMascotAnimationState.Idle),
                animations[SankoMascotAnimationState.Clear] = new SankoMascotAnimation(SankoMascotAnimationState.Clear),
                animations[SankoMascotAnimationState.Kiai] = new SankoMascotAnimation(SankoMascotAnimationState.Kiai),
                animations[SankoMascotAnimationState.Fail] = new SankoMascotAnimation(SankoMascotAnimationState.Fail),
            };
        ...
        private void onNewResult(ValueChangedEvent<JudgementResult?> resultChangedEvent)
        {
            ...
            {
                State.Value = SankoMascotAnimationState.Clear;
        ...
        private SankoMascotAnimationState getNextState()
        {
            ...
            if (!lastObjectHit)
                return SankoMascotAnimationState.Fail;

            return kiaiMode ? SankoMascotAnimationState.Kiai : SankoMascotAnimationState.Idle;
        }

        private void mascotStateChanged(ValueChangedEvent<SankoMascotAnimationState> state)
        {
        ...
        private bool triggerSwellClear(JudgementResult judgementResult)
            => judgementResult.Judgement is SankoSwellJudgement && judgementResult.IsHit;
    }
}
