...
{
    /// <summary>
    /// An overlay that captures and displays osu!sanko mouse and touch input.
    /// </summary>
    public partial class DrumTouchInputArea : VisibilityContainer
    {
        ...
        private KeyBindingContainer<SankoAction> keyBindingContainer = null!;

        private readonly Dictionary<object, SankoAction> trackedActions = new Dictionary<object, SankoAction>();
        ...
        private readonly Bindable<SankoTouchControlScheme> configTouchControlScheme = new Bindable<SankoTouchControlScheme>();

        [BackgroundDependencyLoader]
        private void load(SankoInputManager sankoInputManager, SankoRulesetConfigManager config)
        {
            Debug.Assert(sankoInputManager.KeyBindingContainer != null);

            keyBindingContainer = sankoInputManager.KeyBindingContainer;
            ...
            config.BindWith(SankoRulesetSetting.TouchControlScheme, configTouchControlScheme);
        ...
        private static SankoAction[] getOrderedActionsForScheme(SankoTouchControlScheme scheme)
        {
            switch (scheme)
            {
                case SankoTouchControlScheme.KDDK:
                    return new[]
                    {
                        SankoAction.LeftRim,
                ...
                case SankoTouchControlScheme.DDKK:
                    return new[]
                    {
                        SankoAction.LeftCentre,
                        SankoAction.RightCentre,
                ...
                case SankoTouchControlScheme.KKDD:
                    return new[]
                    {
                        SankoAction.LeftRim,
                        SankoAction.RightRim,
        ...
        private void handleDown(object source, Vector2 position)
        {
            ...
            SankoAction sankoAction = getSankoActionFromPosition(position);

            // Not too sure how this can happen, but let's avoid throwing.
            if (!trackedActions.TryAdd(source, sankoAction))
                return;

            keyBindingContainer.TriggerPressed(sankoAction);
        }
        ...
        private SankoAction getSankoActionFromPosition(Vector2 inputPosition)
        {
        ...
        private partial class DrumSegment : CompositeDrawable, IKeyBindingHandler<SankoAction>
        {
            private SankoAction action;

            public SankoAction Action
            {
                get => action;
                set
                {
            ...
            private void updateColoursFromAction()
            {
                ...
                var colour = getColourFromSankoAction(Action);

                circle.Colour = colour.Multiply(1.4f).Darken(2.8f);
                overlay.Colour = colour;
            }

            private Color4 getColourFromSankoAction(SankoAction handledAction)
            {
