...
namespace osu.Game.Rulesets.Sanko.UI
{
    ...
    internal partial class HitExplosion : PoolableDrawable
    {
        ...
        public HitExplosion(HitResult result)
        {
            ...
            Size = new Vector2(SankoHitObject.DEFAULT_SIZE);
        ...
        private void load()
        {
            InternalChild = skinnable = new SkinnableDrawable(new SankoSkinComponentLookup(getComponentName(result)), _ => new DefaultHitExplosion(result));
        ...
        private static SankoSkinComponents getComponentName(HitResult result)
        {
            switch (result)
            {
                case HitResult.Miss:
                    return SankoSkinComponents.SankoExplosionMiss;

                case HitResult.Ok:
                    return SankoSkinComponents.SankoExplosionOk;

                case HitResult.Great:
                    return SankoSkinComponents.SankoExplosionGreat;
            }
