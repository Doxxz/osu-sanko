...
{
    public sealed partial class SankoMascotAnimation : BeatSyncedContainer
    {
        ...
        public SankoMascotAnimation(SankoMascotAnimationState state)
        {
        ...
        private static TextureAnimation createTextureAnimation(SankoMascotAnimationState state)
        {
            switch (state)
            {
                case SankoMascotAnimationState.Clear:
                    return new ClearMascotTextureAnimation();

                case SankoMascotAnimationState.Idle:
                case SankoMascotAnimationState.Kiai:
                case SankoMascotAnimationState.Fail:
        ...
        {
            private readonly SankoMascotAnimationState state;

            public ManualMascotTextureAnimation(SankoMascotAnimationState state)
            {
                this.state = state;
        ...
        private partial class ClearMascotTextureAnimation : TextureAnimation
        {
            ...
            private void load(ISkinSource source)
            {
                ISkin? skin = source.FindProvider(s => getAnimationFrame(s, SankoMascotAnimationState.Clear, 0) != null);

                if (skin == null) return;

                foreach (int frameIndex in clear_animation_sequence)
                {
                    var texture = getAnimationFrame(skin, SankoMascotAnimationState.Clear, frameIndex);
        ...
        private static Texture? getAnimationFrame(ISkin skin, SankoMascotAnimationState state, int frameIndex)
        {
